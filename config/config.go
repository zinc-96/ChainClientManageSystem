...
type DbConf struct {
	...
	Salt        string `yaml:"salt" mapstructure:"salt"`                   // 盐
	AesKey      string `yaml:"aes_key" mapstructure:"aes_key"`             // aes加密key
	AesIv       string `yaml:"aes_iv" mapstructure:"aes_iv"`               // aes加密iv
...
type Cache struct {
	...
	ChainClientExpired int `yaml:"chain_client_expired" mapstructure:"chain_client_expired"`
}
...
// Chain 长安链信息
type Chain struct {
	ChainID string `yaml:"chaiNid" mapstructure:"chainid"` // 链ID
}

// ChainCA 长安链CA服务配置
type ChainCA struct {
	URL string `yaml:"url" mapstructure:"url"` // 长安链CA服务地址
}

// ChainNode 长安链节点配置
type ChainNode struct {
	ChainNodeURL string `yaml:"chain_node_url" mapstructure:"chain_node_url"` // 长安链节点地址
}
...
type GlobalConfig struct {
	...
	ChainConfig     Chain     `yaml:"chain" mapstructure:"chain"`             // 长安链信息
	ChainCAConfig   ChainCA   `yaml:"chainca" mapstructure:"chainca"`         // 长安链CA服务配置
	ChainNodeConfig ChainNode `yaml:"chainnode" mapstructure:"chainnode"`     // 长安链节点配置
}
